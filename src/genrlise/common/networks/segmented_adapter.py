...
class SegmentedAdapterHyperNetwork(nn.Module):
    """This is a segmented adapter class. This is a more organised version of the `adapter_hypernetwork.py` function.
        In particular, we segment the network into the following
        First layer, middle layers, last layer.
        And we potentially put adapters into either (or both) of the above gaps.
    """
    ...
    def __init__(self,  in_dim: int,
        ...
        def get_hnet(in_dim, out_dim, should_use_activation_fn_at_end=adapter_activation_fn_at_end):
            return Hypernetwork(in_dim=in_dim, out_dim=out_dim, layers=adapter_layers, activation_fn=activation_fn or nn.ReLU(), context_size=context_size, has_activation_func_at_end=should_use_activation_fn_at_end, **hnet_kwargs, use_context_feature_extractor=use_context_feature_extractor, context_feature_extractor_dimension=context_feature_extractor_dimension)
            pass
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, return_generated_weights: bool = False):
        ...
        for i in range(len(self.main_nets)):
            if self.adapters[i] is not None and self.is_adapter_enabled:
                adapted = self.adapters[i](x=features, context=context)
