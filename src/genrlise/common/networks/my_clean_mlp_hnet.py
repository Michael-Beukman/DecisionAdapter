...
class MyCleanHMLP(nn.Module, HyperNetInterface):
    ...
    def forward(self, uncond_input=None, cond_input=None, cond_id=None,
        ...
        if cond_input is not None:
            # print("ABC", cond_input, cond_input.shape, self._cond_in_size)
            assert len(cond_input.shape) == 2 and cond_input.shape[1] == self._cond_in_size, f"Bad sizes here {cond_input.shape} -- {self._cond_in_size}"
            h = cond_input
